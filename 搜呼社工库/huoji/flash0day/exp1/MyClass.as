	...
	public class MyClass
	{
		...
		static public function InitGui(doc: DisplayObjectContainer)
		{
			try
			{
				...
				btn.label = "测试" + (MyUtils.isWin() ? " calc.exe":"");
				btn.move(220, 370);
				btn.setSize(160,26);
				...
				logAdd("Flash版本: " + Capabilities.version + (Capabilities.isDebugger ? " Debug":"")
						+ " " + Capabilities.cpuArchitecture + (is32() ? "-32" : is64() ? "-64":"") + " " + Capabilities.playerType);
				logAdd("操作系统: " + Capabilities.os  + (Capabilities.supports64BitProcesses ? " 64-bit":" 32-bit"));
			    logAdd("提示: 点击测试后,如果弹出计算器则说明您的Flash存在0day! 猎豹、360等谷歌内核浏览器请用IE兼容模式访问!");
